...
from langdata import LANGS, GRID, STAGES, EN, DIP_EN, DIP_LABELS, DATA
...
TTS_FALLBACK = {
    "zh-CN": ["zh"], "zh-HK": ["yue", "zh-TW", "zh"], "pa-IN": ["pa", "hi-IN"],
    "fa-IR": ["fa"], "ar-SA": ["ar"], "ko-KR": ["ko"], "ja-JP": ["ja"],
    "vi-VN": ["vi"], "fil-PH": ["tl-PH", "fil", "tl", "en-PH"],
}
...
def build_phrases(lang):
    ...
    for i, opt in enumerate(dipopts):
        if two:
            rows.append('    {svg:\'%s\', x:"%s", t:"%s", lab:"%s"}' % (DIP_SVGS[i], esc(opt[0]), esc(opt[1]), DIP_LABELS[i]))
        else:
            rows.append('    {svg:\'%s\', x:"%s", lab:"%s"}' % (DIP_SVGS[i], esc(opt[0]), DIP_LABELS[i]))
    out.append(",\n".join(rows) + "\n  ]\n};\n")
    return "".join(out)
...
<style>
  ...
  *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
  ...
  .tile .nat{
    font-size:23px;font-weight:600;color:var(--disc);line-height:1.25;
    display:block;word-break:keep-all;
  }
  .tile .eng{
    margin-top:9px;font-family:var(--f-mono);font-size:8.5px;letter-spacing:.14em;
    text-transform:uppercase;color:var(--taupe);line-height:1.3;
  }
  .tile .draft{
  ...
  @media (max-width:360px){ .tile .nat{font-size:19px} }
</style>
</head>
<body>
<div class="wrap">
...
__TILES__
  ...
  <footer>
    ...
    for the patient. Tap the speaker to play it aloud. Add this page to your home screen and
    it works with no signal.<br><br>
    <strong>Found an error?</strong> <a href="mailto:CONTACT@EXAMPLE.COM?subject=Eyelingual%20correction">Report a correction</a>.
    ...
    for row in GRID:
        for code in row:
            ...
            tiles.append(
                ...
                '      <span class="nat">%s</span>\n'
                '      <span class="eng">%s</span>%s\n'
                '    </a>' % (l["slug"], l["htmlLang"],
                              ' dir="rtl"' if l["dir"] == "rtl" else "",
                              l["native"], l["english"], draft))
