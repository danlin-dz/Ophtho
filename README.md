...
- [ ] `CONTACT@EXAMPLE.COM` in `_template/card.html` and `tools/build.py` → a real
...
Human recordings beat synthesised speech here — TTS flattens prosody, mangles
clinical terms, and device voice coverage is genuinely poor for Cantonese and
Punjabi. The card ships without them and upgrades quietly as they arrive.
